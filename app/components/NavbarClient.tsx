        ...
        <div className="relative w-full max-w-5xl px-4 sm:px-6">
          <div className="grid h-14 grid-cols-3 items-center">
            ...
            <div className="flex items-center">
              ...
              {/* Desktop logo */}
              <Link href="/" className="hidden md:inline text-sm font-semibold tracking-tight text-zinc-900 transition-opacity hover:opacity-60 dark:text-zinc-50">
                Vishnu
              </Link>
            </div>
            ...
            <div className="flex justify-center">
              {/* Mobile logo */}
              <Link href="/" className="md:hidden text-sm font-semibold tracking-tight text-zinc-900 transition-opacity hover:opacity-60 dark:text-zinc-50">
                Vishnu
              </Link>
              {/* Desktop nav */}
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-14 inset-x-0 z-50 border-b border-zinc-200 bg-white/95 backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-950/95 md:hidden"
            >
                    ...
                    >
                      {isBlog ? (
                        <div>
                          <button
                            onClick={() => setBlogOpen(o => !o)}
                            className={`flex w-full h-10 items-center justify-between rounded-lg px-3 text-sm font-medium transition-colors ${
                              isActive || blogOpen
                                ? 'text-zinc-900 bg-zinc-100/80 dark:text-zinc-50 dark:bg-zinc-800/60'
                                : 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800/60 dark:hover:text-zinc-100'
                            }`}
                          >
                            {item.label}
                            <motion.span animate={{ rotate: blogOpen ? 180 : 0 }} transition={{ duration: 0.2 }}>
                              <LuChevronDown size={14} className="text-zinc-400" />
                            </motion.span>
                          </button>
                          <AnimatePresence>
                            {blogOpen && (
                              ...
                              >
                                <div className="pl-3 py-1 flex flex-col gap-0.5">
                                  <Link href="/blog" className="flex h-9 items-center rounded-lg px-3 text-sm text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800/60 dark:hover:text-zinc-100">
                                    All Blogs
                                  </Link>
                                  {pinnedPosts.slice(0, 3).map((post) => (
                                    <Link key={post.slug} href={`/blog/${post.slug}/1`} className="flex h-9 items-center rounded-lg px-3 text-sm text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800/60 dark:hover:text-zinc-100 truncate">
                                      {post.title}
                                    </Link>
                                  ))}
                                </div>
                ...
                <div className="mt-2 border-t border-zinc-100 pt-3 dark:border-zinc-800">
                  <div className="flex items-center gap-2">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex h-10 items-center gap-2 rounded-lg px-3 text-sm text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800/60 dark:hover:text-zinc-100"
                    >
                      <SiGithub size={16} aria-hidden="true" />
                      GitHub
                    </a>
